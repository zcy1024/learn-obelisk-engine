module Suikemon::suikemon_system {
    ...
    fun check_collection(world: &mut World, new_index: u64, new_shiny: bool, player: address) {
        let (mut suikemon_id, mut shiny) = collection_schema::get(world, player);
        ...
        if (!found || shiny[id_index] != new_shiny) {
            ...
            collection_schema::set(world, player, suikemon_id, shiny);
        };
    }
    ...
    fun cancel_from_trading_place(world: &mut World, cancel_index: u64, cancel_number: u64): (u64, bool) {
        let admin = world.admin().to_address();
        let (mut suikemon_id, mut shiny, mut price, mut stock, mut seller) = trading_schema::get(world, admin);
    ...
    public entry fun cancel(world: &mut World, cancel_index: u64, cancel_number: u64, ctx: &mut TxContext) {
        ...
        assert!(seller.length() > cancel_index, ENotExist);
        assert!(seller[cancel_index] == ctx.sender(), ENotSeller);
        let (canceled_suikemon_id, is_shiny) = cancel_from_trading_place(world, cancel_index, cancel_number);
    ...
    public entry fun buy(world: &mut World, buy_index: u64, buy_number: u64, coin: Coin<SUI>, ctx: &mut TxContext) {
        ...
        let (bought_suikemon_id, is_shiny) = cancel_from_trading_place(world, buy_index, buy_number);
